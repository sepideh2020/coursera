...
"""2.Modify the Grass_Pokemon subclass so that the attack strength for Grass_Pokemon instances does not change until they reach level 10. At level 10 and up, their attack strength should increase by the attack_boost amount when they are trained.
...
class Grass_Pokemon(Pokemon):
    ...
    # also we can override train function
    #
    # def train(self):
    #     self.update()
    #     if self.level >= 10:
...
"""3.Along with the Pokemon parent class, we have also provided several subclasses. Write another method in the parent class that will be inherited by the subclasses. Call it opponent. It should return which type of pokemon the current type is weak and strong against, as a tuple.
...
class Flying_Pokemon(Pokemon):
    ...
    def __init__(self, name, level=5):
        super(Flying_Pokemon, self).__init__(name,level)
        self.weak = "Electric"
